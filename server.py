        ...
        async for message in websocket:
            ...
            if message == "/help":
                ...
                if not new_name:
                    ...
                    await websocket.send(f"[{get_time()}] Username '{new_name}' is already taken.")
                else:
                    old_name = username
                    connected_clients[new_name] = connected_clients.pop(old_name)
                    username = new_name
                    rename_msg = f"[{get_time()}] *** {old_name} is now known as {new_name} ***"
                    print(rename_msg)
...
async def main():
    print("TermChat server started on ws://localhost:8765")
